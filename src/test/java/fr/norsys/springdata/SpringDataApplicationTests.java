...
class SpringDataApplicationTests {
    ...
    class Generales {
        ...
        void question7() {
            // TODO : Ecrire la requête SQL
            var query = "SELECT extract(year from age(now(),birthdate)) " +
                    "FROM people " +
                    "where id = 100";
            var typedQuery = entityManager.createNativeQuery(query);
        ...
        void question8() {
            // TODO : Ecrire la requête SQL
            var query = "SELECT AVG(DATE_PART('year', AGE(birthdate))) " +
                    "FROM people";
            var typedQuery = entityManager.createNativeQuery(query);
            var result = typedQuery.getResultList();
        ...
        void question9() {
            ...
            var query = "(SELECT CONCAT(firstname, ',', lastname) AS full_name FROM people ORDER BY LENGTH(firstname) DESC, LENGTH(lastname) DESC LIMIT 1)" +
                    "UNION ALL " +
                    "(SELECT CONCAT(firstname, ',', lastname) AS full_name FROM people ORDER BY LENGTH(lastname) DESC, LENGTH(firstname) DESC LIMIT 1)";
            /**
             * expected
             * Clementine,Carey
             * Wallace,Christensen
        ...
        void question10() {
            ...
            var query = "SELECT concat(firstname ,',',lastname) " +
                    "FROM people " +
                    "ORDER BY length(firstname) + length(lastname) DESC LIMIT 3";
            /** expected
             * Wallace,Christensen
             * Cheyenne,Pennington
             * Isabelle,Singleton
    ...
    class Inviation {
        ...
        void question12() {
            ...
            var query = "SELECT * " +
                    ...
                    "AND DATE_PART('year', AGE(birthdate)) < 60 " +
                    "AND email like '%@%' ";
        ...
        void question13() {
            // TODO : Ecrire la requête SQL
            var query = "SELECT CONCAT(firstname, ',', lastname) AS full_name, DATE_PART('year', AGE(birthdate)) AS age, email " +
                    "FROM people " +
                    "WHERE DATE_PART('year', AGE(birthdate)) > 18 " +
                    "AND DATE_PART('year', AGE(birthdate)) < 60 " +
                    "AND email LIKE '%@%'";
        ...
        void question14() {
            // TODO : Ecrire la requête SQL
            var query = "SELECT concat(firstname ,' ',lastname,' <',email,'>') " +
                    "FROM people " +
                    "WHERE DATE_PART('year', AGE(birthdate)) > 18 " +
                    "AND DATE_PART('year', AGE(birthdate)) < 60";
            /** expected
